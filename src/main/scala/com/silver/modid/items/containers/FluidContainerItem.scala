...
import com.silver.metalmagic.capabilities.EmptyFluidItemHandler
...
object FluidContainerItem {
  def make(capacity: Int, props: Properties, baseID: String, nameFormat: (ITextComponent, ITextComponent) => ITextComponent): List[Item] = {
  ...
  class Full(val capacity: Int, props: Properties, nameFormat: (ITextComponent, ITextComponent) => ITextComponent) extends Item(props.stacksTo(1)) {
    ...
    override def appendHoverText(stack: ItemStack, world : World, text: util.List[ITextComponent], usage: ITooltipFlag): Unit = {
      ...
      withFluidHandler(stack) { handler =>
        ...
        if (fluid.isEmpty) {
          text.add(new TranslationTextComponent("item.metalmagic.container.empty").withStyle(TextFormatting.GRAY))
        } else {
