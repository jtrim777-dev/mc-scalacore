...
import dev.jtrim777.mc.scalacore.inventory.InvSlot
import dev.jtrim777.mc.scalacore.inventory.InvSlot.Generic
...
abstract class MenuBase[D <: MenuDataProvider](kind: MenuType[_], val blockType: Block)
  ...
  protected def layoutPlayerSlots(): Unit = {
    ...
    sx.until(sx + (18*9),18).zipWithIndex.foreach { case (x, i) =>
      addSlot(new InvSlot(playerInventory, i, x, sy + (18*3) + 4, Generic))
    }
